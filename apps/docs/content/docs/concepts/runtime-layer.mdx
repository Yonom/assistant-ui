...
<Files>
  <Folder name="AssistantRuntime" defaultOpen>
    <Folder name="ThreadManagerRuntime" defaultOpen>
...
```tsx
// provides AssistantRuntime, ThreadManagerRuntime, ThreadRuntime, ComposerRuntime (ThreadComposer)
...
const threadManagerRuntime = useAssistantRuntime().threadManager;
...
```tsx
useThreadManager(); // get thread manager state
